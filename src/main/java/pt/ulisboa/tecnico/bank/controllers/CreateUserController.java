...
/**
 * Created with IntelliJ IDEA. User: SONY Date: 12/12/13 Time: 6:58 PM To change
...
public class CreateUserController {
	...
	@PreAuthorize("hasRole('ADMINISTRATOR')")
	public String createUserSubmitForm(
		...
		if (!password.equals(passwordConfirm)) {
			...
			if (role.equals("user"))
				userService.createNormalUser(username, password, "salt",
						"1024", SecurityMatrixUtil.hashMatrix(
								SecurityMatrixUtil.generateMatrix(1000),
								"salt", 1024));
			else if (role.equals("admin"))
				userService.createAdminUser(username, password, "salt", "1024",
						SecurityMatrixUtil.hashMatrix(
								SecurityMatrixUtil.generateMatrix(1000),
								"salt", 1024));
